...
async def top_questions(days: int = Query(30), db: AsyncSession = Depends(get_db)):
    """高频问题 TOP N"""
    svc = AnalyticsService(db)
...
async def intent_distribution(days: int = Query(30), db: AsyncSession = Depends(get_db)):
    """意图分类分布"""
    svc = AnalyticsService(db)
...
async def resolution_trend(days: int = Query(30), db: AsyncSession = Depends(get_db)):
    """AI 解决率 / 转人工率趋势"""
    svc = AnalyticsService(db)
...
async def coverage_heatmap(db: AsyncSession = Depends(get_db)):
    """知识库覆盖率"""
    svc = AnalyticsService(db)
...
async def model_fault_rate(days: int = Query(30), db: AsyncSession = Depends(get_db)):
    """各型号故障率统计"""
    svc = AnalyticsService(db)
...
async def satisfaction_trend(days: int = Query(30), db: AsyncSession = Depends(get_db)):
    """用户满意度走势"""
    svc = AnalyticsService(db)
